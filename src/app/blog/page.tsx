    ...
    <>
      ...
      <section style={{
        ...
        <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 2rem' }}>
          <p style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#C9A84C', marginBottom: '1rem' }}>
            The LatherForge Blog
          </p>
          <h1 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(2.2rem, 5vw, 3.5rem)', fontWeight: 500, color: '#3E2820', lineHeight: 1.2, marginBottom: '1rem' }}>
          ...
          <p style={{ fontSize: '1.05rem', color: '#5C4A3A', lineHeight: 1.8, maxWidth: '560px' }}>
            Free tutorials and practical guides for handmade soap makers.
          </p>
        </div>
      </section>
      ...
      <section style={{ background: '#FAF7F2', padding: '4rem 0 6rem' }}>
        <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '0 2rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
            {posts.map((post, i) => (
              <article key={post.slug} style={{ borderBottom: '1px solid #E8DFD0', padding: i === 0 ? '0 0 3rem' : '3rem 0' }}>
                ...
                <Link href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                  <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 500, color: '#3E2820', lineHeight: 1.3, marginBottom: '0.75rem' }}>
                    {post.title}
                  </h2>
                </Link>
                ...
                <Link href={`/blog/${post.slug}`} style={{ fontSize: '0.85rem', fontWeight: 600, color: '#C9A84C', textDecoration: 'none', letterSpacing: '0.04em', borderBottom: '1px solid rgba(201,168,76,0.4)', paddingBottom: '1px' }}>
                  Read article →
                </Link>
              </article>
            ))}
          ...
          <div style={{ marginTop: '4rem', background: '#3E2820', padding: '3rem', textAlign: 'center' }}>
